...
- 最小 JSON export / import
- import validation 與 local recovery 強化
...
- [x] 明確定義 localStorage migration 策略（目前為 normalize + invalid fallback）
- [x] 補最小 reset / recovery thinking
...
- [x] 對 schema 不符做提示
- [x] 對 version 不符做提示（migration 尚未實作）
...
> 註：這裡以 Jones 單人使用為前提，不需要先做多人 SaaS 級別的 migration framework 或 user preference 分層。
