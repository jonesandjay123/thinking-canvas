...
### 5.4 Gemini / AI 展開體驗
- [x] 從前端 key 遷移到 Cloud Functions
- [x] 使用 `GEMINI_API_KEY` secret
- [x] 完成本地實測打通
- [x] 確認 2nd gen invoke 問題實際解法為 Cloud Run invoker 權限放行
- [ ] retry / backoff
- [ ] fallback model 策略
- [ ] 強化 AI 展開中的等待動畫，讓 spark 按鈕與節點本身在生成期間有更明顯的回饋（例如按鈕態變化、節點邊框動態效果、較清楚的 loading 視覺）
- [ ] 擴充 Gemini prompt context，除了 root 到當前節點的路徑，也納入同層 sibling 節點的標題 / 內容，避免 AI 發想與已存在的兄弟節點重複
- [ ] 先維持目前資料結構，直接用 parent -> childIds 取 sibling context 即可；以 Jones 單人使用規模，短期內不需要為了 sibling 查詢額外做索引或複雜資料結構優化

### 5.5 Firebase Hosting / production 收尾
- [ ] Firebase Hosting 正式部署
- [ ] production env 收斂
- [ ] deploy 流程文件化
